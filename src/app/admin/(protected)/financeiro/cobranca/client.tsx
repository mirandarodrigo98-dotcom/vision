...
export function CobrancaClient({ permissions, isAdminRole = false }: { permissions: string[], isAdminRole?: boolean }) {
    ...
    try {
      ...
      if (res.error) {
        ...
        if (!refresh.error && refresh.data) {
          ...
          setContas(prev => prev.map(c => {
            if (c.codigo_lancamento_omie === selectedRows[0].codigo_lancamento_omie) {
               return { 
                 ...c, 
                 ...refresh.data,
                 valor_pago_calculado: refresh.data.resumo?.valor_pago || refresh.data.valor_pago || 0,
                 status_titulo: refresh.data.status_titulo || 'ATRASADO'
               };
            }
            return c;
          }));
        } else {
          // Recarregar grid principal como fallback
          handleSearch(new Event('submit') as any);
        }
        setIsCarregandoDetalhes(false);
      }
  ...
  const handleEnviarDigisacLote = async () => {
    ...
    for (let i = 0; i < boletosParaEnviar.length; i++) {
      ...
      // Pequeno delay extra de segurança entre contas diferentes (além do que já existe na função)
          ...
          <DialogFooter className="mt-6 flex justify-between items-center border-t pt-4 flex-shrink-0">
            ...
            <div className="flex gap-2">
              <Button 
                ...
                disabled={
                  (!isAdmin && !userPermissions.includes('financeiro.cobranca.contabilidade.receber.cancelar')) || 
                  isCarregandoDetalhes || 
                  !detalheConta ||
                  (() => {
              ...
              <Button 
                ...
                disabled={
                  (!isAdmin && !userPermissions.includes('financeiro.cobranca.contabilidade.receber')) || 
                  isCarregandoDetalhes || 
                  !detalheConta || 
