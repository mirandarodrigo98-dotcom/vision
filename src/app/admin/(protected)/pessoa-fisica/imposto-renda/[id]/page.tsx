...
import { getIRDeclarationById, getIRInteractions, getIRFiles, getIRReceipts } from '@/app/actions/imposto-renda';
...
export default async function DetalhesImpostoRendaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const receipts = await getIRReceipts(id);

  return (
    <div className="flex-1 p-4 md:p-8 pt-6">
      <IRDetails declaration={declaration} interactions={interactions} files={files} receipts={receipts} isAdmin={isAdmin} />
    </div>
  );
}
