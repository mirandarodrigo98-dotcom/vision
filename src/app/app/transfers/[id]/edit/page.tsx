...
export default async function EditTransferPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (transfer.status === 'CANCELLED') {
         return <div>Esta transferência foi cancelada e não pode ser editada.</div>;
    }
    ...
    return (
        <div className="space-y-6">
            <h1 className="text-3xl font-bold tracking-tight">Retificar Transferência</h1>
            <TransferForm companies={companies} initialData={transfer} isEditing={true} />
        </div>
    );
}
