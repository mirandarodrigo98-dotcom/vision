...
import { parseEklesiaCategoriesPDF, saveCategoriesBatch } from '@/app/actions/integrations/eklesia';
...
export function CategoriesImportDialog({ companyId, onSuccess }: CategoriesImportDialogProps) {
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      const res = await parseEklesiaCategoriesPDF(formData, companyId);
      if (res.error) {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const res = await saveCategoriesBatch(result.success, companyId);
      if (res.error) {
        toast.error(res.error);
      } else {
        toast.success(`${res.count} categorias importadas/atualizadas com sucesso!`);
        setIsOpen(false);
        setFile(null);
        setResult(null);
        onSuccess();
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-[800px] w-full max-h-[90vh] flex flex-col p-0 overflow-hidden">
        <div className="p-6 pb-0">
            <DialogHeader>
            <DialogTitle>Importar Categorias via PDF</DialogTitle>
            <DialogDescription>
                Selecione o arquivo PDF de categorias do Eklesia. O sistema importará apenas categorias analíticas (código não zerado).
            </DialogDescription>
            </DialogHeader>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 min-h-[300px]">
          {!result ? (
            <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed rounded-lg bg-muted/50 h-full">
                ...
                <Label htmlFor="cat-pdf-upload" className="cursor-pointer">
            ...
            <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between">
                    <Badge variant="outline" className="border-green-500 text-green-700 bg-green-50 px-3 py-1">
                        ...
                        {result.success.length} Categorias Encontradas
                    </Badge>
                ...
                <div className="border rounded-md overflow-hidden">
                    <Table>
                        <TableHeader className="bg-muted/50">
                            <TableRow>
                                <TableHead className="w-[120px]">Código (Eklesia)</TableHead>
                        ...
                        <TableBody>
                            {result.success.map((item: any, i: number) => (
                                <TableRow key={i}>
                                    ...
                                    <TableCell>
                                        <Badge variant={item.nature === 'Entrada' ? 'default' : 'destructive'} className="text-[10px]">
                                            {item.nature}
                                        </Badge>
                                    </TableCell>
                                </TableRow>
        ...
        <div className="p-4 border-t bg-muted/20 flex justify-end gap-2">
            ...
            >
                ...
                Importar {result?.success?.length || 0} Categorias
            </Button>
        </div>
      </DialogContent>
    </Dialog>
