'use client';

import { useState, useEffect } from 'react';
import {
    Dialog,
    DialogContent,
    DialogDescription,
    DialogHeader,
    DialogTitle,
    DialogTrigger,
} from "@/components/ui/dialog";
import {
    Tooltip,
    TooltipContent,
    TooltipProvider,
    TooltipTrigger,
} from "@/components/ui/tooltip";
import { Button } from "@/components/ui/button";
import { History, Loader2 } from "lucide-react";
import { getVacationHistory } from '@/app/actions/audit';
import { format } from 'date-fns';

interface AuditLog {
    id: string;
    action: string;
    created_at: string;
    metadata: string;
    user_name: string;
    actor_email: string;
}

const ACTION_LABELS: Record<string, string> = {
    'CREATE_VACATION': 'Solicitação de Férias',
    'UPDATE_VACATION': 'Retificação de Férias',
    'CANCEL_VACATION': 'Cancelamento de Férias',
    'APPROVE_VACATION': 'Conclusão de Férias'
};

export function VacationHistory({ vacationId }: { vacationId: string }) {
    const [open, setOpen] = useState(false);
    const [loading, setLoading] = useState(false);
    const [logs, setLogs] = useState<AuditLog[]>([]);

    useEffect(() => {
        if (open) {
            setLoading(true);
            getVacationHistory(vacationId)
                .then(result => {
                    if (result.success && result.logs) {
                        setLogs(result.logs);
                    }
                })
                .finally(() => setLoading(false));
        }
    }, [open, vacationId]);

    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <TooltipProvider>
                <Tooltip>
                    <TooltipTrigger asChild>
                        <DialogTrigger asChild>
                            <Button 
                                variant="outline" 
                                size="sm" 
                                className="text-amber-600 border-amber-200 hover:bg-amber-50"
                            >
                                <History className="h-4 w-4" />
                            </Button>
                        </DialogTrigger>
                    </TooltipTrigger>
                    <TooltipContent>
                        <p>Histórico de Movimentações</p>
                    </TooltipContent>
                </Tooltip>
            </TooltipProvider>
            <DialogContent className="max-w-md max-h-[80vh] flex flex-col">
                <DialogHeader>
                    <DialogTitle>Histórico de Férias</DialogTitle>
                    <DialogDescription>
                        Movimentações e ações registradas.
                    </DialogDescription>
                </DialogHeader>
                
                <div className="flex-1 overflow-y-auto mt-4 pr-2">
                    {loading ? (
                        <div className="flex justify-center py-8">
                            <Loader2 className="h-6 w-6 animate-spin text-gray-400" />
                        </div>
                    ) : logs.length === 0 ? (
                        <p className="text-center text-sm text-muted-foreground py-4">Nenhum registro encontrado.</p>
                    ) : (
                        <div className="space-y-6 relative border-l border-gray-200 ml-3 pl-6 py-2">
                            {logs.map((log) => (
                                <div key={log.id} className="relative">
                                    <span className="absolute -left-[31px] top-1 h-3 w-3 rounded-full bg-gray-200 border-2 border-white ring-1 ring-gray-100" />
                                    <div className="flex flex-col gap-1">
                                        <span className="text-sm font-medium text-gray-900">
                                            {ACTION_LABELS[log.action] || log.action}
                                        </span>
                                        <span className="text-xs text-gray-500">
                                            {format(new Date(log.created_at), 'dd/MM/yyyy HH:mm')}
                                        </span>
                                        <span className="text-xs text-gray-400">
                                            {log.user_name || log.actor_email}
                                        </span>
                                    </div>
                                </div>
                            ))}
                        </div>
                    )}
                </div>
            </DialogContent>
        </Dialog>
    );
}
