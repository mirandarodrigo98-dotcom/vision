...
export function TransferFilters() {
  ...
  const [filters, setFilters] = useState({
    ...
    transfer_date: searchParams.get('transfer_date') || '',
  ...
  const handleFilter = () => {
    ...
    if (filters.transfer_date) params.set('transfer_date', filters.transfer_date);
    ...
    <div className="space-y-4 bg-white p-4 rounded-md border mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="space-y-2 flex flex-col">
            ...
            <DatePicker
                date={parseDate(filters.transfer_date)}
                setDate={(date) => handleFilterChange('transfer_date', date ? format(date, 'yyyy-MM-dd') : '')}
                placeholder="Data de Transferência"
            />
        </div>
      </div>
