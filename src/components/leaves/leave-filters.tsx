...
export function LeaveFilters() {
  ...
  const [filters, setFilters] = useState({
    ...
    leave_date: searchParams.get('leave_date') || '',
  ...
  const handleFilter = () => {
    ...
    if (filters.leave_date) params.set('leave_date', filters.leave_date);
  ...
  return (
    <div className="space-y-4 bg-white p-4 rounded-md border mb-6">
        ...
        <div className="space-y-2">
            ...
            >
                ...
                <SelectContent>
                    ...
                    <SelectItem value="APPROVED">Concluído</SelectItem>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* Leave Date */}
        <div className="space-y-2 flex flex-col">
            <label className="text-sm font-medium">Data do Afastamento</label>
            <DatePicker
                date={parseDate(filters.leave_date)}
                setDate={(date) => handleFilterChange('leave_date', date ? format(date, 'yyyy-MM-dd') : '')}
                placeholder="Data do Afastamento"
            />
        </div>
      </div>
