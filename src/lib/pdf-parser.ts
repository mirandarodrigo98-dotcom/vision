...
export default async function parsePDF(dataBuffer: Buffer, options?: any): Promise<PDFData> {
    ...
    // This forces PDF.js to run in the main thread, which is fine for Node.js
    // @ts-ignore
    ...
    const loadingTask = getDocument({
        data: uint8Array,
        // Disable worker explicitly via options if supported (though GlobalWorkerOptions is main way)
        // isEvalSupported: false is good for security
        isEvalSupported: false,
        // Use system fonts to avoid needing standard_fonts folder
        useSystemFonts: true,
