import path from 'path';
import fs from 'fs';

const envPath = path.resolve(process.cwd(), '.env');
if (fs.existsSync(envPath)) {
  const envConfig = fs.readFileSync(envPath, 'utf8');
  envConfig.split('\n').forEach(line => {
    const match = line.match(/^([^=]+)=(.*)$/);
    if (match) {
      const key = match[1];
      let value = match[2];
      if (value.startsWith('"') && value.endsWith('"')) {
        value = value.slice(1, -1);
      }
      process.env[key] = value.replace(/\r/g, '');
    }
  });
}

async function testDigisacFile() {
  const { getDigisacConfig } = await import('../src/app/actions/integrations/digisac');
  const conf = await getDigisacConfig();
  
  const dummyPdfBase64 = "";

  const filePayload = {
      base64: dummyPdfBase64,
      mimetype: "application/pdf",
      name: "meu_boleto_de_teste",
      extension: "meu_boleto_de_teste"
  };

  try {
      console.log("Uploading file...");
      const uploadResp = await fetch(`${conf.base_url}/api/v1/files`, {
          method: 'POST',
          headers: {
              'Authorization': `Bearer ${conf.api_token}`,
              'Content-Type': 'application/json'
          },
          body: JSON.stringify(filePayload)
      });
      const fileData = await uploadResp.json();
      console.log("Uploaded file ID:", fileData.id);

      const msgPayload = {
          type: "file",
          number: "5524999858882",
          serviceId: conf.connection_phone,
          contactName: "NOVO TESTE NULL TEXT",
          text: null,
          fileId: fileData.id
      };

      console.log("Sending message...");
      const msgResp = await fetch(`${conf.base_url}/api/v1/messages`, {
          method: 'POST',
          headers: {
              'Authorization': `Bearer ${conf.api_token}`,
              'Content-Type': 'application/json'
          },
          body: JSON.stringify(msgPayload)
      });
      console.log("Msg Status:", msgResp.status);
      console.log("Msg Response:", await msgResp.text());
  } catch (e: any) {
      console.log("Error:", e.message);
  }
}
testDigisacFile();